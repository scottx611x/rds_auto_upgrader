...
class RDSPostgresInstance:
    ...
    def __init__(self, db_instance_id, ec2_ip=None, ec2_pass=None, target_version=None):
        ...
        self.refinery_ec2_ip = ec2_ip
        self.refinery_ec2_pass = ec2_pass
    ...
    @property
    def passes_unsupported_usage_checks(self):
        if self.refinery_ec2_ip is None:
            return True
        ssh_client = paramiko.SSHClient()
        ssh_client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        ssh_client.connect(self.refinery_ec2_ip, username="ubuntu",
                           password=self.refinery_ec2_pass)
    ...
    def is_upgradable(self):
        ...
        True
        """
        passes_unsupported_usage_checks = self.passes_unsupported_usage_checks
        if self.target_version is not None:
            return self.uses_postgres and (
                self.target_version in self.upgrade_path
            ) and passes_unsupported_usage_checks
        return self.uses_postgres and passes_unsupported_usage_checks
...
class RDSPostgresUpgrader:
    ...
    def __init__(self, ec2_ip=None, ec2_pass=None,
                 ids=None, tags=None, target_version=None):
        if tags is not None:
...
def create_parser():
    ...
    parser.add_argument(
        '-pass', '--refinery_ec2_pass', type=str,
        help="Password to authenticate ssh connection to Refinery EC2"
    )
    return parser
...
def main():
    ...
    RDSPostgresUpgrader(
        ec2_ip=args.refinery_ec2_ip,
        ec2_pass=args.refinery_ec2_pass,
